...
function CreateRide({isOpen, setIsOpen}) {
  ...
  return (
    <Modal title="Create Ride" centered closable={false} open={isOpen} 
      onOk={() => form.submit()} onCancel={() => handleOnCancel()}
      okText="Create"  cancelText="Cancel" okButtonProps={{disabled: disableOk}}
      cancelButtonProps={{ disabled: disableCancel }} confirmLoading={confirmLoading}>
       <Form {...formItemLayout} form={form} name="create_ride" onFinish={handleSubmit} style={{ maxWidth: 600 }}>
        ...
        {/* vehicle_plate */}
        <Form.Item name="vehicle_plate" label={ <span className="text-gray-700 dark:text-gray-300 font-medium">Vehicle Plate</span>}
          rules={[{ required: true, message: 'Please enter vehicle plate number!' }]} >
          <Input  placeholder="Enter vehicle plate"  className="border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 
          text-gray-900 dark:text-white placeholder-gray-500 dark:placeholder-gray-400"
