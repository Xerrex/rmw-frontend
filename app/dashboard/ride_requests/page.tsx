"use client";

import React, {useState, useEffect} from 'react';
import { PlusIcon } from '@heroicons/react/24/outline';
import { RideRequest } from '@/app/lib/definitions';
import { getRideRequests } from '@/app/lib/data';
import { RideRequestCard } from '../ui/Cards';

function Page() {
  const [ridesRequests, setRidesRequests] = useState<RideRequest[]>([])

  useEffect(()=>{
    getRideRequests()
    .then((data)=>{
      console.log("data", data);
      setRidesRequests(data)
    })
  },[])

  return (
    <div className="flex flex-col flex-1">
      <div className="flex w-full text-black mb-4">
        Ride request filter
      </div>
      <div className="flex flex-wrap w-full h-[780px] overflow-y-auto">
        {ridesRequests.length < 0 && <p className="mx-auto text-black">There are no rides requests yet</p>}
        {ridesRequests.map((rideRequest)=>(<RideRequestCard key={rideRequest.uuid} RideRequest={rideRequest}/>))}
      </div>

      <div className="text-black fixed bottom-20 right-4">
        <button className="bg-primaryColor hover:bg-primaryColorHover text-white p-4 rounded-full shadow-lg  
          transition duration-200"><PlusIcon className="h-8 w-8 text-white" />
        </button>
      </div>
    </div>
  )
}

export default Page